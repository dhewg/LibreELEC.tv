...
addon() {
  ...
  # libxcb
  cp -PL $(get_install_dir libxcb)/.noinstall/libxcb.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXcomposite
  cp -PL $(get_install_dir libXcomposite)/.noinstall/libXcomposite.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXcursor
  cp -PL $(get_install_dir libXcursor)/usr/lib/libXcursor.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXdamage
  cp -PL $(get_install_dir libXdamage)/.noinstall/libXdamage.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXfixes
  cp -PL $(get_install_dir libXfixes)/.noinstall/libXfixes.so.3 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXi
  cp -PL $(get_install_dir libXi)/.noinstall/libXi.so.6 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXrender
  cp -PL $(get_install_dir libXrender)/.noinstall/libXrender.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libxss
  cp -PL $(get_install_dir libxss)/usr/lib/libXss.so.1 $ADDON_BUILD/$PKG_ADDON_ID/lib

  # libXtst
  cp -PL $(get_install_dir libXtst)/.noinstall/libXtst.so.6 $ADDON_BUILD/$PKG_ADDON_ID/lib
