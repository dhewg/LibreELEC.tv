...
parser.add_argument("--with-ninja", type=argparse.FileType('w'),
                    help="build.ninja filename")
...
if args.with_ninja:
    ninja = NinjaWriter(args.with_ninja, width=120)
    ninja.comment('auto generated by %s, do not edit by hand' % sys.argv[0])
    ninja.newline()

    ninja.variable('ROOT', os.getenv('ROOT'))
    ninja.variable('STAMPS', os.getenv('STAMPS'))
    ninja.variable('STAMPS_INSTALL', os.getenv('STAMPS_INSTALL'))
    ninja.variable('LOGS', os.getenv('LOGS'))
    ninja.newline()

    ninja.rule('build',
               'cd $ROOT && scripts/build --nonrecursive $package:$target 2>&1 | tee $LOGS/${package}_${target}.log',
               'Building $package:$target.')
    ninja.newline()
    ninja.rule('install',
               'cd $ROOT && scripts/install --nonrecursive $package:$target 2>&1 | tee $LOGS/${package}_${target}.log',
               'Installing $package:$target.')
    ninja.newline()

    # TODO refactor
    install = True if "image" in args.build else False
    for node in REQUIRED_PKGS:
        node = node.split(':')
        if node[0] == ROOT_PKG:
            continue

        # TODO refactor
        task = "build" if node[1] == ":host" or node[1] == ":init" or not install else "install"
        pkg = ALL_PACKAGES[node[0]]
        deps = pkg.deps[node[1]]

        if task == 'install':
            stamp = '$STAMPS_INSTALL/%s/install_%s' % (node[0], node[1])
        else:
            stamp = '$STAMPS/%s/build_%s' % (node[0], node[1])

        inputs = [ pkg.path + '/package.mk' ]
        for i in deps:
            p = i.split(':')
            if len(p) < 2:
                p.append('target')
            inputs.append('%s_%s_%s' % (task, p[0], p[1]))
        ninja.comment(':'.join(node))
        ninja.build('%s_%s_%s' % (task, node[0], node[1]), 'phony', stamp)
        ninja.build(stamp, task, inputs)
        ninja.variable('package', node[0], indent=1)
        ninja.variable('target', node[1], indent=1)
        ninja.newline()
    ...
    for i in args.build:
        ...
        if len(p) < 2:
            p.append('target')
        task = "build" if p[1] == ":host" or p[1] == ":init" or not install else "install"
        ninja.default('%s_%s_%s' % (task, p[0], p[1]))
